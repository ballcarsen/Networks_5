...
class MPLSFrame:
    ...
    def from_byte_S(self, byte_S):
        ...
        pkt = NetworkPacket.from_byte_S(byte_S[MPLSFrame.label_S_length:])
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        try:
            ...
            try:
                ...
                print('%s: forwarding Decapsulated NetPKT "%s" from interface %d to %d' % (self, pkt, i, out_intf))
            except queue.Full:
            ...
            try:
                ...
                self.intf_L[out_intf].put(l_fr.to_byte_S(), 'out', True)
                print('%s: forwarding frame "%s" from interface %d to %d' % (self, m_fr, i, out_intf))
            except queue.Full:
                print('%s: frame "%s" lost on interface %d' % (self, m_fr, i))
                pass
